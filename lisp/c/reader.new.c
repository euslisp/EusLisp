...
{ int tleng = 0, lengmax=MAXSTRINGLENGTH;      /* '"' is ignored */
  ...
  while ((ch!=terminator) && (ch!=EOF)) {
    ...
    if (tleng >= lengmax) { /*allocate bigger string buffer*/
      ...
      if (newtoken == NULL) {	/*malloc failed*/
...
{ register pointer rdtable;
  ...
  charmacro['(']=makeint(readlist);
  charmacro[')']=makeint(readrparen);
  charmacro['#']=makeint(readsharp);
  charmacro['\'']=makeint(readquote);
  charmacro['"']=makeint(readstring);
  charmacro[';']=makeint(readcomment);

  sharpmacro['\\']=makeint(read_sharp_char);
  sharpmacro['\'']=makeint(read_sharp_function);
  sharpmacro[':']=makeint(read_uninterned_symbol);
  sharpmacro[',']=makeint(read_sharp_eval);
  sharpmacro['.']=makeint(read_sharp_eval);
  sharpmacro['|']=makeint(read_sharp_comment);
  sharpmacro['+']=makeint(read_cond_plus);
  sharpmacro['-']=makeint(read_cond_minus);
  sharpmacro['#']=makeint(readlabref);
  sharpmacro['=']=makeint(readlabdef);
  sharpmacro['(']=makeint(readvector);
  sharpmacro['<']=makeint(read_sharp_object);
  sharpmacro['X']=makeint(read_sharp_hex);
  sharpmacro['O']=makeint(read_sharp_octal);
  sharpmacro['S']=makeint(readstructure);
  sharpmacro['F']=makeint(readfvector);
  sharpmacro['I']=makeint(readivector);
  sharpmacro['J']=makeint(readobject);
  sharpmacro['V']=makeint(readobject);
