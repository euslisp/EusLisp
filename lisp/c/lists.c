...
{ defun(ctx,"CAR",mod,CAR);
  defun(ctx,"CDR",mod,CDR);
  defun(ctx,"REST",mod,CDR);
  defun(ctx,"CADR",mod,CADR);
  defun(ctx,"CDDR",mod,CDDR);
  defun(ctx,"CDAR",mod,CDAR);
  defun(ctx,"CAAR",mod,CAAR);
  defun(ctx,"CADDR",mod,CADDR);
  defun(ctx,"NTH",mod,NTH);
  defun(ctx,"NTHCDR",mod,NTHCDR);
  defun(ctx,"CONS",mod,CONS);
  defun(ctx,"RPLACA",mod,RPLACA);
  defun(ctx,"RPLACA2",mod,RPLACA2);
  defun(ctx,"RPLACD",mod,RPLACD);
  defun(ctx,"RPLACD2",mod,RPLACD2);
  defun(ctx,"APPEND",mod,APPEND);
  defun(ctx,"NCONC",mod,NCONC);
  defun(ctx,"SUBST",mod,SUBST);
  defun(ctx,"NSUBST",mod,NSUBST);
/**/
  defun(ctx,"BUTLAST",mod,BUTLAST);
  defun(ctx,"NBUTLAST",mod,NBUTLAST);
  defun(ctx,"LIST",mod,LIST);
  defun(ctx,"LIST*",mod,LIST_STAR);
  defun(ctx,"MEMQ",mod,MEMQ);
  defun(ctx,"MEMBER",mod,MEMBER);
  defun(ctx,"SUPERMEMBER",mod,SUPERMEMBER);
  defun(ctx,"ASSQ",mod,ASSQ);
  defun(ctx,"ASSOC",mod,ASSOC);
  defun(ctx,"SUPERASSOC",mod,SUPERASSOC);
  }
