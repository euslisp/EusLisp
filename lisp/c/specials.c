...
{ pointer funcname;
  ...
  if (iscode(arg)) return(arg);
    ...
    arg=cons(ctx,makeint(ctx->fletfp),ccdr(arg));
    arg=cons(ctx,makeint(ctx->bindfp),arg);
...
{ register pointer fns, fn;
  ...
  while (iscons(fns)) {	/*allow mutual references between labels functions*/
    fn=ffpp->fclosure;
    fn=ccdr(fn); fn=ccdr(fn); fn=ccdr(fn); ccar(fn)=makeint(ctx->fletfp);
...
{ pointer *spsave=ctx->vsp;
  ...
  cleaner=cons(ctx,makeint(ctx->fletfp),cleaner);
  cleaner=cons(ctx,makeint(ctx->bindfp),cleaner);
